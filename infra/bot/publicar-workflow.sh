...
ARQ="${1:?Uso: bash infra/bot/publicar-workflow.sh caminho/do/workflow.json}"
...
scp -q -o BatchMode=yes -i "$CHAVE" "$ARQ" "$ALVO:/tmp/moedin-wf/workflow.json"

echo "==> 3/5  Parando o n8n, importando e publicando"
"${SSH[@]}" "set -e; cd $PASTA
  docker compose stop n8n >/dev/null
  docker compose run --rm -T -v /tmp/moedin-wf:/import n8n import:workflow --input=/import/workflow.json 2>&1 | grep -E 'Success|rror' || true
...
sleep 6
"${SSH[@]}" "cd $PASTA && docker logs stack-n8n-1 --since 2m 2>&1 | grep -F 'Activated workflow' | grep -F '$ID' | tail -1" \
  || { echo "ATENÇÃO: não vi a linha de ativação de $ID. Confira o log do n8n."; exit 1; }
echo "Pronto: $NOME está publicado e ativo."
